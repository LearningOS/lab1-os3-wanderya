...
lazy_static! {
    ...
    pub static ref TASK_MANAGER: TaskManager = {
        ...
        let mut tasks = [TaskControlBlock {
            ...
            task_syscalls: [0; MAX_SYSCALL_NUM],
            task_start_time: 0,
        }; MAX_APP_NUM];
...
impl TaskManager {
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            inner.tasks[next].task_start_time = get_time_us(); // record start running time 
    ...
    fn incr_syscall_times(&self, id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].task_syscalls[id] += 1;
    }
    ...
    fn get_current_syscall_times(&self) -> [u32; MAX_SYSCALL_NUM] {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].task_syscalls
    }
